...
def plot(currency, raw_data, training_predict, testing_predict, testing_predict_arima, file_name):
    ...
    actual = pyplot.plot(raw_data[int(TRAINING_PERCENTAGE * LENGTH_DATA_SET):], label="Actual rate",
                         color="black")
    testing_rnn = pyplot.plot(testing_data_trend, label="RNN predicted rate", color="green")
    testing_arima = pyplot.plot(testing_predict_arima, label="ARIMA predicted rate", color="red")

    pyplot.ylabel('Rate')
    pyplot.xlabel('Days elapsed')
...
def main():
    data_set_frame = read_csv('forex_16_17_1.csv', header=0,
